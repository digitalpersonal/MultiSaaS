...
export const Sales: React.FC = () => {
  ...
  const [catalog, setCatalog] = useState<any[]>(() => JSON.parse(localStorage.getItem('multiplus_inventory') || '[]'));
  ...
  useEffect(() => { if (flow === 'ORDER') searchInputRef.current?.focus(); }, [flow]);
  ...
  const subtotal = cart.reduce((acc, item) => acc + (item.price || item.salePrice * item.quantity), 0);
  const discountNumeric = parseFloat(discountValue.replace(/\D/g, '')) / 100 || 0;
  const total = Math.max(0, subtotal - discountNumeric);

  const handleFinalize = () => {
    const saleData = { id: `VEN-${Date.now()}`, customer: selectedCustomer || { name: 'Consumidor Final' }, total, paymentMethod, date: new Date().toISOString(), items: cart };
    // Registra no financeiro
    const finance = JSON.parse(localStorage.getItem('multiplus_finance') || '[]');
    localStorage.setItem('multiplus_finance', JSON.stringify([{ id: `FIN-${Date.now()}`, desc: `Venda ${saleData.id}`, amount: total, type: 'INCOME', status: 'PAID', date: saleData.date, method: paymentMethod }, ...finance]));
    setLastSale(saleData); setIsSuccess(true);
  };
