...
export const Dashboard: React.FC = () => {
  ...
  // Reais do banco
  const [inv, setInv] = useState<any[]>(() => JSON.parse(localStorage.getItem('multiplus_inventory') || '[]'));
  const [cust, setCust] = useState<any[]>(() => JSON.parse(localStorage.getItem('multiplus_customers') || '[]'));
  const [os, setOs] = useState<any[]>(() => JSON.parse(localStorage.getItem('multiplus_os') || '[]'));
  const [fin, setFin] = useState<any[]>(() => JSON.parse(localStorage.getItem('multiplus_finance') || '[]'));
  ...
  const stats = useMemo(() => {
    const revenue = fin.filter(f => f.type === 'INCOME').reduce((acc, f) => acc + f.amount, 0);
    const lowStock = inv.filter(p => p.type === 'PHYSICAL' && p.stock <= (p.minStock || 0)).length;
    const openOS = os.filter(o => o.status !== 'COMPLETED' && o.status !== 'CANCELLED').length;
    return { revenue, lowStock, openOS, totalCust: cust.length };
  }, [fin, inv, os, cust]);
      ...
      {showOnboarding && (
        <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[3rem] p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 animate-in slide-in-from-top-6 duration-700 shadow-2xl relative overflow-hidden group">
           <div className="relative z-10 flex items-center gap-8 flex-col md:flex-row text-center md:text-left">
              ...
              <div><h3 className="text-xl font-black text-white">Ambiente Provisionado!</h3><p className="text-indigo-100 font-medium mt-1">Olá {user.name}, complete os dados da sua empresa para habilitar os recursos fiscais.</p></div>
           </div>
